...
// Products page. Always pick products by name, never by index.
export class ProductsPage extends BasePage {
  ...
  // price shown on the product card
  ...
  // adds the product from its card, closes the modal, returns the shown price
  async addToCart(productName: string): Promise<number> {
    ...
    // the add-to-cart anchor has no href, so no link role; match by text
